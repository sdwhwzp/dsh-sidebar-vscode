...
function apply(ctx, input = {}) {
	...
	let authorizing;
	if (tenant !== void 0) ctx.inject(TENANT_SERVICES, (scoped) => {
		authorizing = scoped;
		return () => {
			authorizing = void 0;
		};
	...
	ctx.effect(() => host.webServer.register({
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (method === "ref.take") {
					const record = payload;
					const folder = owned ?? (record !== null && typeof record.folder === "string" ? record.folder : void 0);
					if (folder === void 0 || !folder.startsWith("/")) {
						writeJson(res, 400, {
							ok: false,
							error: {
								code: "bad-request",
					...
					writeJson(res, 200, {
						ok: true,
						value: { envelopes: await takeReferences(spool, folder) }
					});
					return;
				}
