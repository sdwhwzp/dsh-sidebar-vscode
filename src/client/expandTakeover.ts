...
export const EXPAND_BUTTON_SELECTOR = '[data-sidebar-right-expand]'
 ...
 */
export function isExpandControlClick(target: unknown, selector: string = EXPAND_BUTTON_SELECTOR): boolean {
  if (target === null || typeof target !== 'object') return false
  const closest = (target as Partial<ClaimTarget>).closest
  if (typeof closest !== 'function') return false
  try {
    return closest.call(target as ClaimTarget, selector) !== null
  } catch {
    // A hostile or detached target — not ours to claim.
    return false
  }
}
 ...
 */
export function claimExpandClick(
  event: ClaimEvent,
  deps: ExpandTakeoverDeps,
  selector: string = EXPAND_BUTTON_SELECTOR,
): boolean {
  ...
  try {
    ...
    // A refused workbench open must not lose the gesture: the stock
    // expand handler still sees the event.
    return false
  }
...
export interface ListenerTarget {
  addEventListener(type: string, listener: (event: ClaimEvent) => void, options: { capture: boolean }): void
  removeEventListener(type: string, listener: (event: ClaimEvent) => void, options: { capture: boolean }): void
}
 ...
 */
export function installExpandTakeover(
  deps: ExpandTakeoverDeps,
  target: ListenerTarget | undefined = typeof document === 'undefined' ? undefined : document,
  selector: string = EXPAND_BUTTON_SELECTOR,
): () => void {
  if (target === undefined) return () => {}
  const onClick = (event: ClaimEvent): void => {
