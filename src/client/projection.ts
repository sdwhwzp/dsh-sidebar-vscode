...
const defaultTiming: ProjectorTiming = {
 ...
 */
export function overlayZIndex(anchor: ProjectionAnchor): string {
  return anchor.closest?.('[data-sidebar-right-float-host]') != null ? OVERLAY_Z_FLOAT : OVERLAY_Z_DOCKED
}

/** One measured box written onto the host style (idempotent per value). */
function applyRect(host: ProjectionHost, rect: ProjectionRect, zIndex: string): void {
 ...
 */
export class OverlayProjector {
  private anchor: ProjectionAnchor | null = null
  ...
  constructor(
    ...
    private readonly timing: ProjectorTiming = defaultTiming,
  ) {}
